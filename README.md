...
程序跟随最近操作的微信主窗口，并根据该窗口所属进程打开的数据库文件确定微信账号。
它只读取这个账号在程序 `data/<账号>_<路径指纹>/snapshot-<随机ID>/` 下的已验证明文副本：
需有 `contact/contact.db`，同名联系人核对还需 `message/message_*.db`；会话切换辅助线索来自 `session/session.db`。
新账号缺少明文快照时，设置页会显示数据库准备入口；已有快照时自动收起，可用设置标题旁的文件夹图标展开维护。程序只从微信原库复制 DB/WAL 到本地缓存，
后续密钥校验、解密、SQLite 查询全部针对副本。可复用已有本地密钥文件，或点「选择密钥文件」导入；
不会自动提取密钥、扫描微信进程内存，也不会修改原库。准备期间需要约两倍数据库大小的空闲磁盘空间。
首次准备后，运行中的程序约每秒检查当前微信账号的 DB/WAL 变化。日常新增消息只复制 WAL 尾部，
校验已提交帧后增量更新本地明文副本；若 WAL 重置、主库发生无法续接的变化，则只重同步受影响的分库，
这段时间可能超过数秒，不能保证整库始终秒级。校验或密钥失败时保留旧快照并提示，不自动扫描进程内存。
优先读取当前微信窗口 UIA 的完整会话标题和人数，并核对可见的选中行；缺失或冲突时回退 OCR。
当前窗口的 UIA 身份约每 200ms 独立检查一次：标题或可见消息变化后先给出确认状态，
不等待画面停稳和消息 OCR。相同证据的数据库核对结果短暂复用，约每秒重新核对本地快照；
窗口进程自己打开的数据库文件可唯一定位账号时，不再遍历其它微信子进程。
标题只匹配到一条联系人时确认其微信 ID；匹配多条时，须有两条独占的可见消息证据，
或有窗口切换附近新产生的清未读记录加一条独占消息证据，否则保持“未确认”。
群标题中 `I/l/1` 被 OCR 混淆时，插件只在本地联系人副本的群名近似且可见成员数与 `chatroom_member` 完全一致、候选唯一时确认；人数不符或多个相似群仍未确认。
群名被窗口截断时也按可见前缀加成员数核对，只有唯一群符合才确认；OCR 误读人数后的图标不会阻止核对。
企业微信联系人按当前微信账号的 `@openim` 身份与联系人副本匹配；旧快照会在后台更新联系人和会话副本，不会自动重建消息大库。
同名会话优先读取当前窗口 UIA 可见消息与时间；若「1、1、2」这类短消息在本地消息副本中具有唯一的连续顺序和屏幕时间，先确认会话，再运行常规消息 OCR。UIA 不提供足够证据时才按需放大 OCR；旧快照中尚未同步的新消息不能据此确认。
同名候选仍无法自动区分时，回复页会列出该账号快照中的候选备注和微信 ID，须由用户明确选择并点「确认身份」。这是用户确认，不是自动识别；只绑定当前微信窗口与选中的会话行，切换窗口、账号或选中行后失效。填入前还会重新核对当前行和账号；UIA 无法提供稳定选中行时，新消息或画面证据变化也会撤销手选。
历史上最新的清未读记录不代表当前窗口；零未读会话不产生此记录时仍靠其他证据。
未确认时仅展示 OCR 结果，不生成候选或填入。
首次打开会话时，屏幕上已有的消息只作为上下文，不会被当成新消息触发模型请求；此后收到的新消息照常生成回复建议。
确认后，聊天记录会从该副本的对应 `Msg_<md5(username)>` 表按页读取；展开记录后可加载更早消息。
多开时会跟随最近激活的微信窗口；窗口与账号无法唯一关联时不会猜测。
该账号没有完成准备的副本时会保持未确认。
早期没有 `sync.json` 的快照会在后台只重复制并校验当前账号的小型 `session.db/WAL`，为之后发生的清未读变化建立基线；这不更新聊天消息。要启用消息库增量同步和新历史记录，仍需在设置页重新准备一次数据库。
会话识别插件位于 `plugins/session_recognition/`，打包后位于 EXE 旁，可单独替换并在重启后生效；识别策略自由排序和其他插件类型尚未接入。下载页的旧版不包含此改动。
...
<tr>
<td align="center">回复建议：自动跟随当前窗口、群聊可选「回复对象」，3 条候选带 Jev 概率百分比，推荐那条置顶（截图为旧版界面）</td>
...
- **跟着当前会话走**：自动识别微信正在打开的会话，记录、上下文、候选都按会话分开存。
...
- **图片文字**：屏幕上同一张图片里的多行文字会合并展示为「图片文字」；不猜发送方，不凭图片文字自动回复。纯照片的画面语义暂不识别。
- **调试视图**（可选）：另开一个窗口，实时画出截到的画面和每个识别框——绿 = 我、蓝 = 对方、
  灰 = 过滤掉的灰字、橙 = 当成发言人名、红 = 图片文字候选、黄 = 小字候选，外加消息区和头部的框、
...
主界面上的采集开关、同名候选确认、「回复对象」下拉和「填入时带 @」勾选框只在内存里，
不落盘，重启回默认。

## 界面说明

- **会话跟随**：界面始终显示微信当前打开的会话；聊天记录、模型上下文和候选按会话隔离。同名且无法自动区分时，才显示「同名候选」供手动确认。
- **回复对象（群聊，可选）**：设置里打开「群聊指定回复对象」，群聊时会出现一行下拉框，
...
- **输入框拉高超过面板一半会认错消息区**：消息区靠「面板 45% 高度以下第一根分隔线」定位，
  输入框拉太高就会把它当成消息区底线。
- **OCR 的「文字必须落在平底色上」规则只对精确像素的帧成立**：框里众数颜色占比低于 45% 就当成图片文字候选，
  只有较大的连续文字区域才展示，头像的小字仍过滤。缩放或压缩过的图（比如拿预览窗再截一次）底色会糊成几百种颜色，
  整屏都会被当成图片。
